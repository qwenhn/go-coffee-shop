...
import (
	...
	events "github.com/qwenhn/go-coffee-shop/internal/pkg/event"
...
func appendLineItems(
	...
	for _, item := range items {
		product, ok := lo.Find(itemTypes, func(i *ItemModel) bool {
		...
		lineItem := NewLineItem(
			...
			shared.StatusInProcess,
			isBarista,
		)

		if isBarista {
			order.ApplyDomain(events.BaristaOrdered{
				OrderID:    order.ID,
				ItemLineID: lineItem.ID,
				ItemType:   item.ItemType,
			})
		} else {
			order.ApplyDomain(events.KitchenOrdered{
				OrderID:    order.ID,
				ItemLineID: lineItem.ID,
				ItemType:   item.ItemType,
			})
...
func CreateOrderFrom(ctx context.Context, request *PlaceOrderModel, productDomainSvc ProductDomainService) (*Order, error) {
	...
	if numberOfBaristaItems {
		itemTypes, err := productDomainSvc.GetItemsByType(ctx, request, true)
		if err != nil {
			return nil, err
		}

		appendLineItems(order, request.BaristaItems, itemTypes, true)
	}

	if numberOfKitchenItems {
		itemTypes, err := productDomainSvc.GetItemsByType(ctx, request, false)
		if err != nil {
			return nil, err
		}

		appendLineItems(order, request.KitchenItems, itemTypes, false)
	}
...
func (o *Order) Apply(event *events.OrderUp) error {
	if len(o.LineItems) == 0 {
		return nil // we don't do anything
	}
