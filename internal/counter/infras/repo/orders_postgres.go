...
import (
	...
	"github.com/qwenhn/go-coffee-shop/internal/counter/infras/postgresql"
...
func (o *orderRepo) GetAll(ctx context.Context) ([]*domain.Order, error) {
	...
	lineItems := lo.FilterMap(results, func(x postgresql.GetAllRow, _ int) (*domain.LineItem, bool) {
		if !x.Price.Valid || !x.ItemType.Valid || !x.Name.Valid || !x.ItemStatus.Valid || !x.IsBaristaOrder.Valid {
			return nil, false
		}

		priceX, err := strconv.ParseFloat(x.Price.String, 32)
		if err != nil {
			return nil, false
		}
		...
		return &domain.LineItem{
			...
			ItemType:       shared.ItemType(x.ItemType.Int32),
			Name:           x.Name.String,
			Price:          float32(priceX),
			ItemStatus:     shared.Status(x.ItemStatus.Int32),
			IsBaristaOrder: x.IsBaristaOrder.Bool,
			OrderID:        x.ID,
		}, true
	})
